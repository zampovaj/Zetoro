    ...
    @php
        ...
        if ($mode === 'create') {
            $modalSubtitle = match($type) {
                'file' => 'Add new file to the selected article.',
                'article', 'folder' => 'Add new ' . $type . ' to the ' . ($parentId ? 'selected folder.' : 'root workspace.'),
                'annotation' => 'Add new annotation to the open file.',
                default => 'Create new ' . $type . '.'
            };
        } else {
            $modalSubtitle = 'Edit selected ' . $type . '.';
        }
    @endphp
        ...
        @elseif($type === 'file')
            ...
            @if ($mode === 'create')
                <div x-data="{ isDropping: false }" x-on:dragover.prevent="isDropping = true"
                    ...
                    <input type="file" id="pdf-upload" wire:model="fileForm.uploadFile" class="hidden"
                        accept=".pdf" />
